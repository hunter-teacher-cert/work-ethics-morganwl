"""Creates an empty plane and seats randomly generated passengers based
on the company's seating algorithm."""
...
def get_avail_seats(plane,economy_sold):
    ...
    Parameters: plane : a list of lists representing plaine
                economy_sold : a dictionary of the economy seats sold
                but not necessarily assigned

    Returns: the number of unsold seats

    Notes: this loops over the plane and counts the number of seats that
    are "avail" or "win" 
...
def seat_economy_block(plane, name, seats):
    """Seats a block of economy passengers, trying to put them together
    if possible."""
    rows = len(plane)
    cols = len(plane[0])

    # look for a block of seats on the plane
    for row_i in range(rows):
        block_size = 0
        for seat_i in range(cols):
            if plane[row_i][seat_i] == 'avail' or plane[row_i][seat_i] == 'win':
                block_size += 1
            else:
                block_size = 0
            if block_size == seats:
                seat_i += 1
                for seat_j in range(seat_i - seats, seat_i):
                    plane[row_i][seat_j] = name
                print(f'Successfully seated {seats} seats for {name}.')
                return plane

    # if no suitable block is found, seat passengers anywhere we can
    print(f'No block found for {seats} seats for {name}, ' +
          'seating anywhere available.')
    return seat_economy(plane, dict(), name)
...
def fill_plane(plane):
    ...
    # appending a number to either "ep" for economy plus or "u" for
    # unassigned economy seat
    ep_number=1
    u_number=1

    # MODIFY THIS
    # you will probably want to change parts of this
    # for example, when to stop purchases, the probabilities, maybe the
    # size for the random
    ...
    while total_seats > 1:
        ...
        if r > 30:
            ...
            economy_sold = purchase_economy_block(
                plane, economy_sold, 1 +
                random.randrange(max_family_size), "u-%d"%u_number)
            u_number = u_number + 1
    ...
    for name, seats in economy_sold.items():
        plane = seat_economy_block(plane, name, seats)
